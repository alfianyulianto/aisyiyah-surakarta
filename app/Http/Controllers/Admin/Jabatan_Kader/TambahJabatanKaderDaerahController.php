...
class TambahJabatanKaderDaerahController extends Controller
{
  ...
  public function destroy(Request $request, Kader $kader, $id)
  {
    ...
    return redirect('/jabatan/kader/daerah/' . $id)->with('message_delete_pimp_daerah', 'Berhasil menghapus ' . $kader->nama . ' sebagai ' . $request->jabatan . '.');
  }
  ...
  public function get_jabatan(Periode $periode, Daerah $daerah)
  {
    ...
    foreach ($user as $u) {
      ...
      if (Kader::where('nik', $u->kader_nik)->where('daerah_id_daerah', $daerah->id_daerah)->first() && !KaderJabatan::where('periode_id_periode', $periode->id_periode)->where('kader_nik', $u->kader_nik)->first()) {
